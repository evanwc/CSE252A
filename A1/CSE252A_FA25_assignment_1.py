...
# Answer:
# $\\$
# x lies on point l so: $l^\top x = 0 \\$
# scalars transpose does nothing, scalar multiplication is commutative so: $x^\top l = l^\top x \\$
# therefore $x^\top l = 0$ and $l^\top x = 0 \\$
# so $x^\top l + l^\top x = 0$
...
# Answer:
# $\\\\$
# There is a point $x$ in which intersects both lines: $l_1^\top x = 0$ and $l_2^\top x = 0 \\\\$
# $l_1 \times l_2$ is a vector that is orthogonal to both $l_1$ and $l_2$ so $x = l_1 \times l_2$
# 
...
def to_homog(points_inhomog):
    ...
    ones = np.ones((1, points_inhomog.shape[1]))
    homo_points = np.vstack((points_inhomog, ones))
...
def from_homog(points_homog):
    ...
    """
    #### Write your code here. ####
    denoms = points_homog[-1,:]
    inhomog_points = points_homog[:-1,:] / denoms
...
def project_points(M_int, M_ext, pts):
    ...
    cam_pts = M_ext @ pts_h
    img_pts = M_int @ cam_pts

    pts_2d = from_homog(img_pts)
...
def camera1():
    #### Write your code here. ####
    P_int_proj = intrinsic_cam_mat(2)

    t = np.array([0, 0, 0])
    P_ext = extrinsic_cam_mat([0, 0, 0], t)
...
def camera3():
    ...
    t = np.array([0, 1, 2])
    P_ext = extrinsic_cam_mat([0, 0, np.pi/6], t)

    return P_int_proj, P_ext

def camera4():
    #### Write your code here. ####
    P_int_proj = intrinsic_cam_mat(4)

    t = np.array([0, 0, 3])
    P_ext = extrinsic_cam_mat([np.pi/6, 0, np.pi/4], t)
...
# 
# Answer: The distortion comes from the angle of the camera on the object in a way that is not aligned with the z-axis. This looks distorted because the image has an extreme perspective where the vanishing points create an irregular quadrilateral shape.
# 
...
def plot_normals(diffuse_normals, original_normals, mask):
    ...
    diff_sphere = normal2rgb(np.array(diffuse_normals[0]))
    diff_sphere[mask[0] == 0] = [0, 0, 0]
    plot1.imshow(diff_sphere)

    orig_sphere = normal2rgb(np.array(diffuse_normals[1]))
    orig_sphere[mask[0] == 0] = [0, 0, 0]
    plot2.imshow(orig_sphere)

    diff_pear = normal2rgb(np.array(diffuse_normals[2]))
    diff_pear[mask[1] == 0] = [0, 0, 0]
    plot3.imshow(diff_pear)

    orig_pear = normal2rgb(np.array(diffuse_normals[3]))
    orig_pear[mask[1] == 0] = [0, 0, 0]
    plot4.imshow(orig_pear)
...
#### Write your code here. ####
plot_normals(data_normal, data_normal, data_mask)
...
def lambertian(normals, light, color, intensity, mask):
    ...
    s_hat = light / np.linalg.norm(light)
    s = intensity * s_hat

    n_dot_s = np.dot(normals, s)
    maximum = np.maximum(0, n_dot_s)
    
    albedo = np.ones((H, W))

    image = albedo[..., None] * color[None, None, :] * maximum[..., None]
...
# computed by:
# 
# $$e(x,y) = s_a k_a(x,y) + \sum_{m\in M} s_m (k_d(x,y) f_d(x,y) + k_s(x,y) f_s(x,y)) \text{,}$$
# 
...
def blinn_phong(normals, ambient, lights, colors, material, view, mask):
    ...
    v_hat = view / np.linalg.norm(view)
    
    image = np.zeros((H, W, 3))

    ambient = np.array(ambient)
    
    image += ka * ambient[None, None, :]

    lights = np.array(lights)
    for i in range(lights.shape[0]):
        s_hat = lights[i] / np.linalg.norm(lights[i])
        c = np.array(colors[i])

        n_dot_s = np.dot(normals, s_hat)
        f_d = np.maximum(0, n_dot_s)
        diffuse = kd * c[None, None, :] * f_d[..., None]

        h_x_y = s_hat + v_hat
        h_hat = h_x_y / np.linalg.norm(h_x_y)

        n_dot_h = np.dot(normals, h_hat)
        f_s = np.maximum(0, n_dot_h)
        specular = ks * c[None, None, :] * (f_s[..., None] ** a)
...
results_sphere = [
    lambertian(data_normal[0], [-0.5,0.5,0.5], [0.1,0.4,0.8], 1, data_mask[0]),
    lambertian(data_normal[0], [0.3,-0.2,0.5], [0.7,0.3,0.3], 1, data_mask[0]),
    lambertian(data_normal[1], [-0.5,0.5,0.5], [0.1,0.4,0.8], 1, data_mask[0]),
    lambertian(data_normal[1], [0.3,-0.2,0.5], [0.7,0.3,0.3], 1, data_mask[0])
]

results_pear_diffuse = [
    blinn_phong(data_normal[2], [1,1,1], [[-0.5,0.5,0.5]], [[0.1,0.4,0.8]], [0.1,0.2,0.6,3], [0,0,1], data_mask[1]),
    blinn_phong(data_normal[2], [1,1,1], [[-0.5,0.5,0.5]], [[0.1,0.4,0.8]], [0.1,0.6,0.2,3], [0,0,1], data_mask[1]),
    blinn_phong(data_normal[2], [1,1,1], [[-0.5,0.5,0.5]], [[0.1,0.4,0.8]], [0.1,0.6,0.6,10], [0,0,1], data_mask[1]),
    blinn_phong(data_normal[2], [1,1,1], [[0.3,-0.2,0.5]], [[0.7,0.3,0.3]], [0.1,0.2,0.6,3], [0,0,1], data_mask[1]),
    blinn_phong(data_normal[2], [1,1,1], [[0.3,-0.2,0.5]], [[0.7,0.3,0.3]], [0.1,0.6,0.2,3], [0,0,1], data_mask[1]),
    blinn_phong(data_normal[2], [1,1,1], [[0.3,-0.2,0.5]], [[0.7,0.3,0.3]], [0.1,0.6,0.6,10], [0,0,1], data_mask[1]),
    blinn_phong(data_normal[2], [1,1,1], [[-0.5,0.5,0.5],[0.3,-0.2,0.5]], [[0.1,0.4,0.8],[0.7,0.3,0.3]], [0.1,0.2,0.6,3], [0,0,1], data_mask[1]),
    blinn_phong(data_normal[2], [1,1,1], [[-0.5,0.5,0.5],[0.3,-0.2,0.5]], [[0.1,0.4,0.8],[0.7,0.3,0.3]], [0.1,0.6,0.2,3], [0,0,1], data_mask[1]),
    blinn_phong(data_normal[2], [1,1,1], [[-0.5,0.5,0.5],[0.3,-0.2,0.5]], [[0.1,0.4,0.8],[0.7,0.3,0.3]], [0.1,0.6,0.6,10], [0,0,1], data_mask[1])
]

results_pear_original = [
    blinn_phong(data_normal[3], [1,1,1], [[-0.5,0.5,0.5]], [[0.1,0.4,0.8]], [0.1,0.2,0.6,3], [0,0,1], data_mask[1]),
    blinn_phong(data_normal[3], [1,1,1], [[-0.5,0.5,0.5]], [[0.1,0.4,0.8]], [0.1,0.6,0.2,3], [0,0,1], data_mask[1]),
    blinn_phong(data_normal[3], [1,1,1], [[-0.5,0.5,0.5]], [[0.1,0.4,0.8]], [0.1,0.6,0.6,10], [0,0,1], data_mask[1]),
    blinn_phong(data_normal[3], [1,1,1], [[0.3,-0.2,0.5]], [[0.7,0.3,0.3]], [0.1,0.2,0.6,3], [0,0,1], data_mask[1]),
    blinn_phong(data_normal[3], [1,1,1], [[0.3,-0.2,0.5]], [[0.7,0.3,0.3]], [0.1,0.6,0.2,3], [0,0,1], data_mask[1]),
    blinn_phong(data_normal[3], [1,1,1], [[0.3,-0.2,0.5]], [[0.7,0.3,0.3]], [0.1,0.6,0.6,10], [0,0,1], data_mask[1]),
    blinn_phong(data_normal[3], [1,1,1], [[-0.5,0.5,0.5],[0.3,-0.2,0.5]], [[0.1,0.4,0.8],[0.7,0.3,0.3]], [0.1,0.2,0.6,3], [0,0,1], data_mask[1]),
    blinn_phong(data_normal[3], [1,1,1], [[-0.5,0.5,0.5],[0.3,-0.2,0.5]], [[0.1,0.4,0.8],[0.7,0.3,0.3]], [0.1,0.6,0.2,3], [0,0,1], data_mask[1]),
    blinn_phong(data_normal[3], [1,1,1], [[-0.5,0.5,0.5],[0.3,-0.2,0.5]], [[0.1,0.4,0.8],[0.7,0.3,0.3]], [0.1,0.6,0.6,10], [0,0,1], data_mask[1])
]
...
def photometric_stereo(images, lights, mask, horn_niter=25000):
    ...
    albedo = np.linalg.norm(b, axis=0).reshape(height, width)

    normals = (b / np.linalg.norm(b, axis=0, keepdims=True)).T.reshape(height, width, 3)

    nx = b[0, :].reshape(height, width)
    ny = b[1, :].reshape(height, width)
    nz = b[2, :].reshape(height, width)
    p = -nx/nz
    q = -ny/nz
    ...
    albedo = np.where(mask, albedo, 0)
    normals = np.where(mask[..., None], normals, 0)
    H = np.where(mask, H, 0)
    H_horn = np.where(mask, H_horn, 0)
...
lights = np.stack([
    np.squeeze(data['l1']), 
    np.squeeze(data['l3']), 
    np.squeeze(data['l4'])], axis=0)

images = np.stack([data['im1'], data['im3'], data['im4']], axis=-1)
...
images = np.stack([data['im1'], data['im2'], data['im3'], data['im4']], axis=-1)
...
def RGBToSUV(I_rgb, rot_vec):
    ...
    rot_vec_norm = rot_vec / np.linalg.norm(rot_vec)

    R = get_rot_mat(rot_vec_norm)
    ...
    H, W, _ = I_rgb.shape
    I = I_rgb.reshape(-1,3).T

    S_channel = S @ I
    U_channel = U @ I
    V_channel = V @ I

    S = S_channel.reshape(H, W)
    U = U_channel.reshape(H,W)
    V = V_channel.reshape(H,W)
...
def plot_RGB_SUV(filename, imtitle):
    ...
    I_rgb = data['im1']
    rot_vec = np.array([1.0, 1.0, 1.0])

    S, D = RGBToSUV(I_rgb, rot_vec)
...
def to_gray(image):
    blue = image[:,:,2] * 0.072192319
    green = image[:,:,1] * 0.71516871
    red = image[:,:,0] * 0.21263903
    gray = red + green + blue
    return gray

images = np.stack([to_gray(data['im1']), to_gray(data['im2']), to_gray(data['im3']), to_gray(data['im4'])], axis=-1)

avg_intensity = np.mean(images, axis=-1)
img_mask = normalize(avg_intensity)
mask = (img_mask > 0.15).astype(np.float32)

albedo, normals, depth, horn = photometric_stereo(images, lights, mask, 10000)
visualize(albedo, normals, depth, horn, "Photometric Stereo - Sphere", 5)
...
_, D1 = RGBToSUV(img1, rot_vec)
_, D2 = RGBToSUV(img2, rot_vec)
_, D3 = RGBToSUV(img3, rot_vec)
_, D4 = RGBToSUV(img4, rot_vec)

images_clean = np.stack([D1, D2, D3, D4], axis=-1)

albedo_clean, normals_clean, depth_clean, horn_clean = photometric_stereo(images_clean, lights, mask, 10000)
visualize(albedo_clean, normals_clean, depth_clean, horn_clean, "Photometric Stereo - Sphere Specularities Removed", 5)
...
avg_intensity = np.mean(images, axis=-1)
img_mask = normalize(avg_intensity)
mask = (img_mask > 0.1).astype(np.float32)

albedo, normals, depth, horn = photometric_stereo(images, lights, mask, 10000)
visualize(albedo, normals, depth, horn, "Photometric Stereo - Pear", 5)
...
albedo_clean, normals_clean, depth_clean, horn_clean = photometric_stereo(images_clean, lights, mask, 10000)
visualize(albedo_clean, normals_clean, depth_clean, horn_clean, "Photometric Stereo - Pear Specularities Removed", 5)
